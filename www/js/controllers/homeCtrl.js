angular.module('imageID.controllers').controller('HomeCtrl', function($scope, $timeout, $state, $log, Restangular, MemoryService, $rootScope, $ionicSlideBoxDelegate, $ionicHistory) {
    ...
    $scope.next = function(handle) {
        $ionicSlideBoxDelegate.$getByHandle(handle).next();
    }

    $scope.previous = function(handle) {
        $ionicSlideBoxDelegate.$getByHandle(handle).previous();
    }

	$scope.goToDetailsProduct = function(v){
		MemoryService.putData("product", v);
		$state.go("app.product", { "id" : v.id });
	}
	
	$scope.goToDetailsDesigner = function(v){
		MemoryService.putData("designer", v);
		$state.go("app.designer", { "id" : v.id });
	}
	
	$scope.goToDetailsShowroom = function(v){
		MemoryService.putData("showroom", v);
		$state.go("app.showroom", { "id" : v.id });
	}
	
	$scope.goToDetailsCatalog = function(v){	
		var url = v.file;
	...
	$scope.goToDetailsHouse = function(v){
