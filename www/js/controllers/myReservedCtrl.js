angular.module('imageID.controllers').controller('MyReservedCtrl', function($scope, $timeout, $state, AuthService, Restangular,MemoryService, WORDPRESS_REST_BASE_URL, $http, $ionicLoading) {
	$scope.data = {};
	...
	$http.get(WORDPRESS_REST_BASE_URL + '/user/' + $scope.data.user.id + '/profile').then(function(posts) {
		...
		console.log(posts.data.reserved);
