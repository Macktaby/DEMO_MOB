angular.module('imageID.services')
  ...
  this.register = function(userId) {
    if(ionic.Platform.isIOS())
    {
      ...
      $cordovaPush.register(ios_config).then(function(result) {
        ...
        WpPushServer.storeDeviceToken("ios", result, userId);
      }, function(err) {
        ...
        switch(notification.event)
        {
          case 'registered':
            if (notification.regid.length > 0 ) {
              console.log("notification.regid: " + notification.regid);
              WpPushServer.storeDeviceToken("android", notification.regid, userId);
            }
            break;
          ...
          case 'message':
            ...
            var post_type = notification.payload["gcm.notification.post_type"];
            var post_id = notification.payload["gcm.notification.post_id"];
            if(post_type == "designer") {
                $state.go("app.designer", { "id" : post_id });
            } else if(post_type == "vendor") {
                $state.go("app.vendor", { "id" : post_id });
            } else if (post_type == "showroom"){
                $state.go("app.showroom", { "id" : post_id });
            } else if (post_type == "house"){
                $state.go("app.house", { "id" : post_id });
            } else if (post_type == "product"){
                $state.go("app.product", { "id" : post_id });
            }
